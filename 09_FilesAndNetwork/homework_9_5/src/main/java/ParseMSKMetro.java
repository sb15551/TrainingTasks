...
public class ParseMSKMetro {
    ...
    public static void create(String whereFrom, String whereTo) {
        try {
            elements = getTable(whereFrom);
            initStationsAndLines(whereFrom);
            initConnections(whereFrom);
            ...
            FileWriter write = new FileWriter(whereTo, false);
            write.write(json);
            write.flush();
            write.close();
        } catch (Exception e) {
     ...
     * @throws IOException
     */
    private static Elements getTable(String url) throws IOException {
     ...
     * @throws IOException
     */
    private static void initStationsAndLines(String whereFrom) throws IOException {
     ...
     */
    private static void initConnections(String whereFrom) {
                ...
                if (!connectStation.select("span").isEmpty()) {
                    ...
                    for (Element span : connectStation.select("span")) {
                        ...
                        if (!span.attr("title").isEmpty()) {
                            ...
                            String stationNull = getStationConnect(lineNumberTemp, lineStationTemp);
                            if (stationNull.equals("null")) {
                                try {
                                    throw new IllegalArgumentException("Not found station on line");
                                }
                                catch (Exception e) {
                                    System.out.println(e.getMessage());
                                }
                            } else {
                                connect.add(new Connections(lineNumberTemp, stationNull));
                            }
                        }
                    }
                    Collections.sort(connect);
                    if (!connections.contains(connect) && !lineNumberTemp.equals("011А")) {
